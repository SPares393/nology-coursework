...
var playersAlreadySelected = []; // Get correct players
...
for (var i = 0; i < mainCard.length; i++) {
  mainCard[i].addEventListener("click", function () {
    currentCard = this;
    playerList.innerHTML = ""; // Set current player card
    ...
    var listCloseButton = document.querySelector(".list-close-button");
    listCloseButton.addEventListener("click", toggleHideList);
  });
} // Fill card based on list selection


playerList.addEventListener("click", function (e) {
  if (e.target.classList.contains("player-list-button")) {
    playersAlreadySelected.push(e.target.textContent.split(" "));
